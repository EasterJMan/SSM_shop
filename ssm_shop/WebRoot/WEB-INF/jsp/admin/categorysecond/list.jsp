...
<script type="text/javascript">
		...
		function edit(csid) {
			window.location.href = "${pageContext.request.contextPath}/admin/adminCategorySecond_edit.action?csid="+csid;
		}
		function deletecs(csid) {
			window.location.href ="${pageContext.request.contextPath}/admin/adminCategorySecond_delete.action?csid="+csid;
		}
		</script>
</head>
<body>
		...
		<tr align="center">
			<td colspan="4">第 ${csPageBean.page }/${csPageBean.totlePage}页 <c:if
					...
					<a
			     	...
			     	<a
						href="${pageContext.request.contextPath }/admin/adminCategorySecond_findAllByPage.action?page=${csPageBean.page-1}">上一页</a>
				</c:if>
				 <c:forEach begin="1" end="${csPageBean.totlePage}" var="i">
					<c:choose>
						<c:when test="${csPageBean.page!=i}">
							<a
								href="${ pageContext.request.contextPath }/admin/adminCategorySecond_findAllByPage.action?page=${i}">${i}</a>
						</c:when>
						<c:otherwise>
				...
				<c:if test="${csPageBean.page!=csPageBean.totlePage}">
					<a
						href="${pageContext.request.contextPath }/admin/adminCategorySecond_findAllByPage.action?page=${csPageBean.page+1}">下一页</a> |
							<a
						href="${pageContext.request.contextPath }/admin/adminCategorySecond_findAllByPage.action?page=${csPageBean.totlePage}">尾页</a>
				</c:if>
			</td>
		</tr>
	</table>
